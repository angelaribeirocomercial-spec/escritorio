...
- `artifact_path` (required) â€” Path to HTML file or course directory
- `--viewports` (optional) â€” Viewport sizes: `1920x1080,768x1024,375x812` (default)
- `--output` (optional) â€” Directory to save screenshots (default: `/tmp/visual-qa/`)
...
Use browser dev tools (F12) â†’ Device Emulation to change viewports.
...
**Status:** âœ… PASS / âš ï¸ WARN / âŒ FAIL
...
- âœ… Screenshots captured at all required viewports
- âœ… Visual defects identified and categorized
- âœ… Report clearly documents status (PASS/WARN/FAIL)
- âœ… If FAIL, specific remediation steps provided
- âœ… If issues fixed, re-inspection confirms resolution
...
- Chrome/Edge: F12 â†’ Device Emulation (Ctrl+Shift+M)
