...
type ProcessCockpitFrameProps = {
  ...
  processFilings: {
    ...
    records: ReadonlyArray<{
      ...
      nextAction: string;
      updatedAt: string;
      contextualMinutaHref?: string;
      contextualMinutaLabel?: string;
    }>;
  };
};
                  ...
                  <div key={record.id} className="detail-soft-row px-4 py-4 text-sm text-slate-300">
                    <div className="flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
                      <div>
                        ...
                        {record.contextualMinutaHref ? (
                          <Link
                            className="detail-link-button mt-3 inline-flex px-3 py-2 text-xs font-semibold"
                            href={record.contextualMinutaHref}
                          >
                            {record.contextualMinutaLabel ?? "Abrir minuta contextual"}
                          </Link>
                        ) : null}
                      </div>
                      <span className="rounded-[4px] border border-white/10 bg-white/[0.04] px-3 py-1 text-xs font-semibold text-slate-200">
