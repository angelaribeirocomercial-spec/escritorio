import Link from "next/link";

import { WorkspacePage } from "@/components/layout/workspace-page";

const crmActions = [
  {
    label: "Clientes",
    href: "/pessoas/clientes",
    detail: "Abrir a carteira ativa com contexto juridico e comercial do escritorio."
  },
  {
    label: "Adversos",
    href: "/pessoas/adversos",
    detail: "Listar bancos, partes contrarias e contexto util para a operacao."
  },
  {
    label: "Contatos / Partes",
    href: "/pessoas/contatos-partes",
    detail: "Navegar pelas pessoas vinculadas ao fluxo atual de relacionamento."
  }
] as const;

export default function CrmPage() {
  return (
    <WorkspacePage
      description="O CRM concentra relacionamento, carteira ativa e contexto de onboarding sem quebrar as rotas atuais do workspace."
      eyebrow="CRM"
      metrics={[
        { label: "Foco", value: "Relacionamento" },
        { label: "Compatibilidade", value: "Rotas atuais" },
        { label: "Slice", value: "Navegacao" },
        { label: "Estado", value: "Ativo" }
      ]}
      title="Carteira e relacionamento do escritorio"
    >
      <section className="grid gap-4 lg:grid-cols-3">
        {crmActions.map((item) => (
          <Link
            key={item.href}
            className="workspace-soft-card flex h-full flex-col justify-between rounded-[4px] border border-white/10 bg-white/[0.04] p-5 transition hover:bg-white/[0.07]"
            href={item.href}
          >
            <div>
              <p className="text-lg font-semibold text-white">{item.label}</p>
              <p className="mt-2 text-sm leading-6 text-slate-300">{item.detail}</p>
            </div>
            <span className="mt-4 inline-flex w-fit rounded-[4px] border border-emerald-300/20 bg-emerald-300/10 px-3 py-1 text-xs font-semibold text-emerald-100">
              Abrir area
            </span>
          </Link>
        ))}
      </section>
    </WorkspacePage>
  );
}
